...
import { API_URL, DEFAULT_SITE_DATA } from "../data.js";
...
export default function AdminPage({ siteData, onSave, onExit }) {
  ...
  async function handleUnlock() {
    ...
    try {
      ...
      if (res.ok) {
        // Pull the freshest saved data into the editor before unlocking
        try {
      ...
      setError(
        "Couldn't reach the server. This only works on the live Netlify site, not when previewing locally."
      );
    } finally {
      setChecking(false);
    }
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch(API_URL, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ passcode, data: cleaned }),
      });
      if (res.ok) {
        setDraft(cleaned);
        onSave(cleaned);
        setStatusMessage({
          kind: "success",
          text: "Saved! These changes are now live for every visitor.",
        });
      } else if (res.status === 401) {
        setStatusMessage({
          kind: "error",
          text: "Session problem: passcode no longer accepted. Reload the page and unlock again.",
        });
      } else {
        setStatusMessage({
          kind: "error",
          text: "The server rejected the save. Try again in a moment.",
        });
      }
    } catch {
      setStatusMessage({
        kind: "error",
        text: "Couldn't reach the server — check your internet connection and try again. Your edits are still here.",
      });
    } finally {
      setSaving(false);
    }
      ...
      <main className="flex min-h-screen items-center justify-center bg-ink px-4">
        <div className="w-full max-w-sm rounded-xl border border-edge bg-panel p-8">
          ...
          <button
            ...
            disabled={checking}
            className="mt-4 w-full rounded-md bg-crimson py-3 text-sm font-bold tracking-widest text-white transition hover:bg-crimson-dark disabled:opacity-60"
          >
      ...
      <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-4 space-y-2">
            {Object.entries(draft.menu).map(([category, items]) => {
              ...
              return (
                ...
                >
                  <button
                    onClick={() => setOpenCategory(isOpenCat ? "" : category)}
        ...
        <div className="sticky bottom-4 mt-6 flex flex-col gap-3 sm:flex-row">
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 rounded-md bg-crimson py-3.5 text-sm font-bold tracking-widest text-white shadow-lg shadow-ink transition hover:bg-crimson-dark disabled:opacity-60"
          >
            {saving ? "SAVING..." : "SAVE & PUBLISH TO EVERYONE"}
          </button>
          <button
            onClick={handleReset}
        ...
        {statusMessage.text && (
          <p
            className={`mt-3 text-center text-sm ${
              statusMessage.kind === "success" ? "text-jade" : "text-crimson"
            }`}
          >
            {statusMessage.text}
          </p>
